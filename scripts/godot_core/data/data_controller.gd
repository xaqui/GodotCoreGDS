...
func load_game():
	...
	#data_persistance_objects = find_all_data_persistance_objects()
#	for dpo in data_persistance_objects:
#		dpo.load_data(game_data)
	log_msg("Loaded Game Gata.")

func save_game():
	# Pass the data to other scripts so they can update it
	#data_persistance_objects = find_all_data_persistance_objects()
	get_tree().call_group("data_persistance_objects","save_game",game_data)
#	for dpo in data_persistance_objects:
#		dpo.save_data(game_data)
...
func configure():
	#data_persistance_objects = find_all_data_persistance_objects()
	file_handler = FileDataHandler.new(DATA_PATH,SAVE_FILENAME, false)
	
#func find_all_data_persistance_objects():
#	return get_tree().get_nodes_in_group("data_persistance_objects")
