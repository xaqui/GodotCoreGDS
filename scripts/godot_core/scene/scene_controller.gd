...
func _ready():
	...
	log_msg(str("Current Scene Path: ",current_scene().get_scene_file_path()))
	
# Public Functions
func load_scene(_scene : SceneType.type, reload : bool = false, _loading_page : PageType.type = PageType.type.none):
	...
	if(!scene_can_be_loaded(_scene, reload)):
		log_warning(str("Tried to load scene [",SceneType.type.keys()[_scene],"] but it cannot be loaded."))
		return
	# Initialization
...
func deferred_go_to_scene(path):
	...
	await wait_for_scene_load(path)
...
func wait_for_scene_load(path):
	...
	while(ResourceLoader.load_threaded_get_status(path, load_progress) == ResourceLoader.THREAD_LOAD_IN_PROGRESS):
		...
		#log_msg(str("Loading...",load_progress[0]))
...
func on_scene_loaded(_scene : Node):
	var scene_type = scene_path_to_type(_scene.get_scene_file_path())
	if(target_scene != scene_type): return
	
	# Send signal notifying all interested parties that the scene has finished loading
	scene_loaded.emit(scene_type)
...
func scene_can_be_loaded(_scene : SceneType.type, reload : bool) -> bool:
	...
	if(target_scene_path == ""):
		...
		return false
	elif(current_scene().get_scene_file_path() == target_scene_path && !reload):
...
func scene_path_to_type(_path : String) -> SceneType.type:
	...
	return SceneType.type.get(scene_type)
